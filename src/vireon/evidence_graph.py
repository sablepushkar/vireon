...
class EvidenceGraph:
    ...
    def add_signal(self, signal: Signal) -> None:
        ...
        self.edges.append(GraphEdge(signal.event_id, signal.signal_id, "produced_signal"))
    ...
    def add_evidence(self, evidence: EvidenceLink) -> None:
        ...
        self.edges.append(GraphEdge(evidence.signal_id, evidence.evidence_id, "supported_by"))

    def as_dict(self) -> dict[str, object]:
        return {
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "nodes": [{"id": n.node_id, "type": n.node_type, "label": n.label} for n in self.nodes.values()],
            "edges": [{"source": e.source, "target": e.target, "relationship": e.relationship} for e in self.edges],
        }
